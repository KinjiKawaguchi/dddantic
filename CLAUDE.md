...
> Status: v0.1 implemented — core building blocks (ValueObject, Entity, AggregateRoot, DomainEvent), the registry, and Mermaid diagramming. Specification and Repository are deferred. The following are the finalized design decisions behind the implementation.
...
constraints/      Invariant checks for each element (called from __init_subclass__)
registry/         Element registration and lookup
diagram/          Mermaid output (PlantUML in the future)
specification/    Specification and combinators
...
- **Repository**: Constrains its type parameter to an AggregateRoot subclass. Targets AggregateRoot only.
- **Specification**: Composable via `is_satisfied_by` + `&` / `|` / `~`.
